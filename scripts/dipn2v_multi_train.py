...
def run_training(train_paths:list):
    ...
    steps_per_epoch = 360
    # Get the absolute path of the directory containing the .tif files
    tif_files_directory = os.path.abspath(os.path.join(baseDir, 'datasets', 'training'))

    # Get the list of .tif files
    train_paths = [os.path.join(tif_files_directory, file) if not os.path.isabs(file) else file for file in train_paths]
    ...
    generator_test_param["train_path"] = os.path.join(tif_files_directory, train_paths[0])
    ...
    generator_param_list = [{**generator_param, "train_path": path} for path in train_paths]
    ...
    return training_param["model_string"]

if __name__ == "__main__":    
    model_name = run_training()
